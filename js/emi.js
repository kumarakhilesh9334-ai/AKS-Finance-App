...
function parseSheetDate(d) {
  ...
  // Format: ISO timestamp "2026-08-21T12:57:15.479Z" — parse the full moment.
  // Never slice UTC date parts: a date entered as 14-Aug IST is stored as
  // 2026-08-13T18:30:00Z in UTC, so slicing gives the wrong day.
  if (/^\d{4}-\d{2}-\d{2}T/.test(s)) {
    const isoDt = new Date(s);
    return isNaN(isoDt) ? null : isoDt;
  }
